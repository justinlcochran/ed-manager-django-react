...
function ChartColumn({title, popUpText, setFieldValues, fields, placeholder}) {
    ...
    const handleAdd = () => {
        let newTextArea = [...textAreas, {colType:title, textAreaId: textAreas[textAreas.length-1].textAreaId+1, placeholder: placeholder}]
        ...
        <div>
            <div>
                ...
                <button className="inline-flex bg-lime-600 max-w-max rounded font-bold px-2 hover:bg-lime-700" type={"button"} onClick={() => handleAdd()} name={title}>
                    <img alt='' src="https://ucarecdn.com/fba38ac8-7b37-4a34-abcc-afca5c76c53d/Untitleddesign.png" className={"h-5 w-5 p-1 justify-center"}/>
                </button>
